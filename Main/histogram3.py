...
if not os.path.isfile("C:/users/cxyre/Desktop/blocks.pickle") and not os.path.isfile("C:/users/cxyre/Desktop/labels.pickle"):
    #csv
    skeleton_csv = pd.read_csv('D:/user/Documents/Skripsi/Dataset/fix/train_newest11.csv')
    skeleton_name = []
    for i in tqdm(range(len(skeleton_csv))):
        skeleton_name.append(skeleton_csv['image'][i].split('_')[0])

    df = pd.DataFrame()
    df['skeleton'] = skeleton_name
    df = df.drop_duplicates()
    df.to_csv('D:/user/Documents/Skripsi/Dataset/fix/skeleton_newest.csv', index=False)
...
#                 if blocks[count][math.ceil(k*1.0/8.0) - 1][math.ceil(j*1.0/8.0) - 1] == frame_count:
#                     continue
#                 if frame[k, j][1] >= 150:
#                     # print(frame[k, j])
#                     blocks[count][math.ceil(k*1.0/8.0) - 1][math.ceil(j*1.0/8.0) - 1] = blocks[count][math.ceil(k*1.0/8.0) - 1][math.ceil(j*1.0/8.0) - 1] + 1

#     f = open(os.path.join('C:/users/cxyre/Desktop/', 'block.pickle'), "wb")
#     f.write(pickle.dumps(blocks))
#     f.close()

#     f = open(os.path.join('C:/users/cxyre/Desktop/', 'label.pickle'), "wb")
#     f.write(pickle.dumps(blocks_class))
#     f.close()

# block = pickle.loads(open(os.path.join('C:/users/cxyre/Desktop/', 'block.pickle'), "rb").read())
# label = pickle.loads(open(os.path.join('C:/users/cxyre/Desktop/', 'label.pickle'), "rb").read())
...
# # for i in range(28):
# #   for j in range(28):
# #     if blocks[n][i][j] <= blocks[m][i][j] + 1 and blocks[n][i][j] >= blocks[m][i][j] - 1:
# #       similiar = similiar + 1
    
# # print('Total: %i' % similiar)
# # print('Persen: %.02f%%' % (similiar*100.0/(28*28)))

# # print('Total data: %i' % len(block))
...
#                             sim += 1
#                 similiar[i].append(sim * 100 / (28 * 28))
